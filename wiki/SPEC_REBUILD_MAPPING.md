...
| 阶段 | 交付 | 退出条件 | 状态 |
|---|---|---|---|
| **A0** | 阻断性缺陷修复 + 回归脚本 | 4 项设备复现全部转绿；单测全绿 | 未开始 |
| A | 删除入口清单、损坏处理器、停用未保护清理 | 能说明每种删除入口；故障测试副本不被自动删掉 | 未开始 |
| B | DatabaseExecutor、待办聚合事务、显式 SaveResult | 先删后插中断不丢旧内容；界面不忽略保存失败 | 未开始 |
| C | **v9** 加法迁移、CAS、operations、历史 | 旧数据保真；并发修改可检测；历史可恢复 | 未开始 |
| D | SaveCoordinator、EditorViewModel、不可变附件 | 乱序/离开/失败/图片移除测试通过 | 未开始 |
| E | 逻辑快照、ZIP+manifest、全量校验、SAF | 全新目录恢复并逐项验证成功 | 未开始 |
