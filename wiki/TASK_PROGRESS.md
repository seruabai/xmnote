...
- T-021 | **富文本编辑器第二批：数据库迁移到 v3 块文档（DB_VERSION 9→10）**。`data/Db.kt` 新增 `< 10` 迁移分支（逐行搬运 + 单行容错：转换失败的行保持旧版本号，读取端仍能解析）；并入 `BODY_FORMAT_BLOCKS = 3` 与 `COL_BODY_FORMAT`；写入路径**显式**写 `body_format_version`（此前只靠 DDL 默认值，是漏标版本的根源），全新安装默认值同步为 3。新增唯一格式闸口 `core/NoteBody.kt`（按行内版本解码 + 双向格式嗅探 + v1 私有标记先过 `migrateBodyV1toV2`）；`NoteRepository` 读写改走闸口；`BackupCodec` 导入把任意来源版本统一规范化成 v3。设备测试 `SchemaMigrationTest` 同步更新（v8→v10、v1→v10、全新安装三条用例断言改为 v10 + 块文档）并新增 v9→v10 专项（文本/清单/已升级三类行 + 幂等复开）。单测 284 全绿；**androidTest 仅编译通过，未在设备上运行**。遗留：旧版 APK 读不了新备份包（v3 正文），是否做兼容待用户定 | DeepSeek Harness | 第二批完成（设备验证待跑） | data/Db.kt, core/NoteBody.kt, data/NoteRepository.kt, backup/BackupCodec.kt, app/src/test/.../core/NoteBodyTest.kt, app/src/androidTest/.../data/SchemaMigrationTest.kt | 2026-09-16 |
