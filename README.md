...
> **当前版本不申请网络权限、不发起任何网络请求。** 规划中的云同步（WebDAV）只会把备份写到**你自己的云盘**，不经中间服务器，也不做任何遥测或统计上传。
...
- **数据以本机为准**：全部数据存在设备上，离线完整可用；**当前版本不申请网络权限、不发起任何请求**
- **云同步（规划中）只写你自己的云盘**：WebDAV 直连你的存储，不经中间服务器；除同步外**无任何上传**（无遥测、无统计、无账号体系）
