...
> 一款小米笔记风格的**本地优先** Android 笔记应用。**数据始终以本机为准**：离线完整可用、无需登录，应用不开启云同步时不会发起任何网络请求。云同步只把备份写到**你自己的云盘**（WebDAV 直连），中间没有我们的服务器，也没有任何遥测或统计上传。
...
- **数据以本机为准**：全部数据存在设备上，离线完整可用；关闭云同步时应用不发起任何网络请求
- **云同步只写你自己的云盘**：通过 WebDAV 直连你的存储，没有中间服务器；除同步外**无任何上传**（无遥测、无统计、无账号体系）
...
> 唯一的功能性框架依赖是 **WorkManager**（自动备份的周期调度）；数据层、备份、编辑管线仍为手写实现。
